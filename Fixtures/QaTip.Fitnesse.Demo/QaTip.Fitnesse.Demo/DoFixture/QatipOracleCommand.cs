...
{
    public sealed class QatipOracleCommand
    {
        public QatipOracleCommand(CommandType _commandType, string _commandText, params object[] parameterValues)
        {
	...
	/// <summary>
	/// OracleHelper encapsulates all common methods used by application DALs.
    /// This is based on MS's DAAB.
	/// </summary>
	public sealed class OracleHelper
	{
		...
		{
            CoreHelpers.LogMessage(string.Format("Executing sql against: {0}", connectionString));
        ...
        /// <summary>
        /// Execute an set of OracleCommands (that returns no resultset and takes no parameters) in one transaction against the provided OracleTransaction. 
        /// </summary>
        /// <remarks>
        /// e.g.:  
        ///  int result = ExecuteNonQuery(connectionString, TPPOracleCommand1, TPPOracleCommand2);
        /// </remarks>
        /// <param name="connectionString">a valid Oracle connection string</param>
        /// <param name="commands">an array of TPPOracleCommand which contains CommandType, CommandText, and Parameters for stored procedure only</param>
        /// <returns>an int representing the number of rows affected by the last command</returns>
        public static int ExecuteNonQuery(string connectionString, params QatipOracleCommand[] commands)
        {
            CoreHelpers.LogMessage(string.Format("Executing sql against: {0}", connectionString));
            var ret = 0;
            //create & open an OracleConnection, and dispose of it after we are done.
            using (var cn = new OracleConnection(connectionString))
            {
                ...
                OracleCommand cmd;
                foreach (var TPPCommand in commands)
                {
                    //create a command and prepare it for execution
                    cmd = new OracleCommand();

                    OracleParameter[] commandParameters = null;
                    //if we got stored procedure parameter values, we need to figure out where they go
                    if ((TPPCommand.ParameterValues != null) && (TPPCommand.ParameterValues.Length > 0))
                    {
                        if (TPPCommand.CommandType == CommandType.StoredProcedure)
                        {
                            //pull the parameters for this stored procedure from the parameter cache (or discover them & populate the cache)
                            commandParameters = OracleHelperParameterCache.GetSpParameterSet(connectionString, TPPCommand.CommandText);

                            //assign the provided values to these parameters based on parameter order
                            AssignParameterValues(commandParameters, TPPCommand.ParameterValues);
                        }
                    }
                    PrepareCommand(cmd, cn, transaction, TPPCommand.CommandType, TPPCommand.CommandText, commandParameters);
